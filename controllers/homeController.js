		...
		dbconnection.query(sql_client_upcoming_workouts, [request.session.dbId], function(error, upcoming_results){
			...
			dbconnection.query(sql_workout_list, [request.session.dbId], function(error, workouts_results){
				...
				dbconnection.query(sql_friends, [db_id, db_id ], function(error, results){
					if(error) throw error;
					response.render(path.join(__dirname, "../views/clientViews/clientHome"), {firstName: firstName, upcoming: upcoming_results, friends: results, workout_list: workout_list, role: request.session.role})
				})
			})
		});	
